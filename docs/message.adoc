...
// define a callback for my_message
...
// the callback is the same as before
constexpr auto my_callback = msg::callback<"my_callback", my_message_defn>(
    [](msg::const_view<my_message_defn>) { /* do something */ });
...
NOTE: This section documents the details of the `indexed_service`. It's not required
to understand this to _use_ an `indexed_service`.
...
expression]. The `callback` construction process ensures that this
...
The initialization process when `callback`​s are added to the builder
