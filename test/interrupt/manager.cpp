...
namespace interrupt {
    ...
    public:
        ...
        MOCK_METHOD(void, init, (std::size_t irq_number, std::size_t priorityLevel));
    ...
    struct MockIrqImpl {
        ...
        static void run(std::size_t irq_number, Callable interrupt_service_routine) {
            StatusPolicy::run(
                [&]{
                    callbackPtr->status(irq_number);
                },
                [&]{
    ...
    struct mock_field_t {
        ...
        constexpr static Reg get_register() {
            return {};
        }
    ...
    template<int Id, typename Reg, typename Name>
    ...
    struct read_op_t {
        ...
        auto operator()() const {
            return callbackPtr->read(field.id);
        }
    };

    template<typename FieldType>
    constexpr auto read(FieldType field) {
    ...
    struct write_op_t {
        ...
        void operator()() const {
            values.for_each([](auto value){
                callbackPtr->write(value.id, value.value);
            });
        }
    };

    template<typename... ValueTypes>
    constexpr auto write(ValueTypes... values) {
        return write_op_t<ValueTypes...>{cib::make_tuple(values...)};
    }

    template<typename... FieldTypes>
    constexpr auto clear(FieldTypes... fields) {
        return write_op_t<field_value_t<FieldTypes>...>{cib::make_tuple(field_value_t<FieldTypes>{0}...)};
    }

    template<typename... OpTypes>
    constexpr void apply(OpTypes... ops) {
        (ops(), ...);
    }
    ...
    struct int_sts_reg_t : mock_register_t<0, int_sts_reg_t, decltype("int_sts"_sc)> {};
    struct packet_avail_sts_field_t : mock_field_t<1, int_sts_reg_t, decltype("packet_avail_sts"_sc), 1, 1> {};
    struct rsp_avail_sts_field_t : mock_field_t<2, int_sts_reg_t, decltype("rsp_avail_sts"_sc), 0, 0> {};

    struct int_en_reg_t : mock_register_t<3, int_en_reg_t, decltype("int_en"_sc)> {};
    struct packet_avail_en_field_t : mock_field_t<4, int_en_reg_t, decltype("packet_avail_en"_sc), 1, 1> {};
    struct rsp_avail_en_field_t : mock_field_t<5, int_en_reg_t, decltype("rsp_avail_en"_sc), 0, 0> {};

    #define EXPECT_WRITE(FIELD_TYPE, VALUE) EXPECT_CALL(callback, write(FIELD_TYPE::id, VALUE))
        ...
        using Config =
            root<
                ...
                shared_irq<33, 0, policies<clear_status_first>,
                    sub_irq<
                        packet_avail_en_field_t,
                        packet_avail_sts_field_t,
                        msg_handler_irq,
                        policies<required_resources<test_resource_alpha>>>,
                    sub_irq<
                        rsp_avail_en_field_t,
                        rsp_avail_sts_field_t,
                        rsp_handler_irq,
                        policies<required_resources<test_resource_alpha, test_resource_beta>>>>,
    ...
    TEST_F(InterruptManagerTest, NoIsrInit) {
        constexpr auto manager =
            NoIsrBuilder::value.build<NoIsrBuilder>();
    ...
    static auto constexpr bscan = flow::action("bscan"_sc, [] {
        callbackPtr->run(0xba5eba11);
    });

    struct hwio_int_sts_field_t : mock_field_t<6, int_sts_reg_t, decltype("hwio_int_sts_field_t"_sc), 2, 2> {};
    struct i2c_int_sts_field_t : mock_field_t<7, int_sts_reg_t, decltype("i2c_int_sts_field_t"_sc), 3, 3> {};
    struct spi_int_sts_field_t : mock_field_t<8, int_sts_reg_t, decltype("spi_int_sts_field_t"_sc), 4, 4> {};
    struct can_int_sts_field_t : mock_field_t<9, int_sts_reg_t, decltype("can_int_sts_field_t"_sc), 5, 5> {};

    struct hwio_int_en_field_t : mock_field_t<10, int_en_reg_t, decltype("hwio_int_en_field_t"_sc), 2, 2> {};
    struct i2c_int_en_field_t : mock_field_t<11, int_en_reg_t, decltype("i2c_int_en_field_t"_sc), 3, 3> {};
    struct spi_int_en_field_t : mock_field_t<12, int_en_reg_t, decltype("spi_int_en_field_t"_sc), 4, 4> {};
    struct can_int_en_field_t : mock_field_t<13, int_en_reg_t, decltype("can_int_en_field_t"_sc), 5, 5> {};
    ...
    TEST_F(InterruptManagerTest, BasicManagerSharedSubIrqRun) {
        constexpr auto manager =
            BasicBuilder::value.build<SharedSubIrqTest>();
