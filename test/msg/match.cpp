...
namespace {
    ...
    template<int Value>
    static constexpr Number<Value> number{};
    ...
    TEST_CASE("MatchAnyDescription", "[match]") {
        REQUIRE(
            match::any(number<1>).describe() ==
            "number == 1"_sc);

        REQUIRE(
            match::any(number<1>, number<4>).describe() ==
            "(number == 1) || (number == 4)"_sc);

        REQUIRE(
            match::any(number<1>, number<2>, number<3>).describe() ==
    ...
    TEST_CASE("MatchAllDescription", "[match]") {
        REQUIRE(
            match::all(number<1>).describe() ==
            "number == 1"_sc);

        REQUIRE(
            match::all(number<1>, number<4>).describe() ==
            "(number == 1) && (number == 4)"_sc);

        REQUIRE(
            match::all(number<1>, number<2>, number<3>).describe() ==
            "(number == 1) && (number == 2) && (number == 3)"_sc);
    }
