...
using test_msg_t = message_base<decltype("test_msg"_sc), 4, 2,
                                test_id_field, test_opcode_field,
                                test_field_2, test_field_3>;


using index_spec = decltype(cib::make_tuple(
    cib::index_metafunc_<get_field_type>,
    cib::index_metafunc_<get_identity>,
...
static constexpr auto test_callback = msg::indexed_callback_t(
...
static constexpr auto test_callback_equals = msg::indexed_callback_t(
    "TestCallback"_sc, test_id_field::equal_to<0x80>,
    [](test_msg_t const &) { callback_success = true; });

struct test_project_equals {
    constexpr static auto config = cib::config(
        cib::exports<test_service>, cib::extend<test_service>(test_callback_equals));
};
...
static constexpr auto test_callback_multi_field =
    msg::indexed_callback_t(
        "test_callback_multi_field"_sc,

        match::all(
            test_id_field::in<0x80, 0x42>,
...
static constexpr auto test_callback_single_field =
    msg::indexed_callback_t(
        ...
        [](test_msg_t const &) {
            callback_success_single_field = true;
        }
    );
...
struct test_project_multi_field {
    constexpr static auto config = cib::config(
        ...
        cib::extend<test_service>(test_callback_multi_field, test_callback_single_field)
    );
};
...
TEST_CASE("build handler multi fields", "[handler_builder]") {
    ...
    cib::service<test_service>->handle(test_msg_t{test_id_field{0x80}, test_opcode_field{1}});
    REQUIRE(callback_success);
    ...
    callback_success = false;
    callback_success_single_field = false;
    cib::service<test_service>->handle(test_msg_t{test_id_field{0x50}, test_opcode_field{1}});
    REQUIRE_FALSE(callback_success);
