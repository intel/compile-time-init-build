...
namespace msg {
    ...
    struct message_data {
        ...
        [[nodiscard]] constexpr bool operator==(message_data const & rhs) const {
            return
                this->num_dwords == rhs.num_dwords &&
                this->data == rhs.data;
        }
    ...
    template<typename MsgType, typename additional_matcher>
    struct is_valid_msg_t {
        ...
        template<typename BaseMsgType>
        [[nodiscard]] constexpr auto describe_match(BaseMsgType const & base_msg) const {
            MsgType const msg{base_msg.data};
    ...
    struct message_base : public message_data<MaxNumDWords> {
        ...
        [[nodiscard]] static constexpr auto match(additional_matcherType additional_matcher) {
        ...
        static constexpr auto match_valid_encoding = [](){
            constexpr auto required_fields =
                cib::filter(FieldTupleType{}, [](auto field){
                    ...
                    return sc::bool_<!std::is_same_v<match::always_t<true>, std::decay_t<decltype(FieldType::match_requirements)>>>;
                });
            ...
            if constexpr (required_fields.size() == 0) {
                ...
                return required_fields.apply([](auto... required_fields_pack) {
                    return match::all(decltype(required_fields_pack)::match_requirements ...);
                });
            }
        }();
            ...
            : message_data<MaxNumDWords>{}
        {
            ...
            if constexpr (sizeof...(argFields) == 0) {
                ...
                if constexpr (detail::is_iterable<decltype(first_arg)>) {
                    std::copy(std::begin(first_arg), std::end(first_arg), std::begin(this->data));
                    ...
                    arg_field_tuple.for_each([&](auto field) {
                        set(field);
                    });
                }
            }
        ...
        [[nodiscard]] constexpr auto describe() const {
            ...
            const auto middle_string =
                field_descriptions.fold_left([](auto lhs, auto rhs){
                    return lhs + ", "_sc + rhs;
                });
