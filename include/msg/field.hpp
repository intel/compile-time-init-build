...
namespace msg {
    ...
    public:
        ...
        static constexpr void fits_inside(MsgType) {
        ...
        template<T expected_value>
        static constexpr msg::greater_than_t<This, T, expected_value> greater_than{};

        template<T expected_value>
        static constexpr msg::greater_than_or_equal_to_t<This, T, expected_value> greater_than_or_equal_to{};

        template<T expected_value>
        static constexpr msg::less_than_t<This, T, expected_value> less_than{};

        template<T expected_value>
        static constexpr msg::less_than_or_equal_to_t<This, T, expected_value> less_than_or_equal_to{};

        template<T NewGreaterValue>
        using WithGreaterThan = field<NameTypeT, DWordIndexT, MsbT, LsbT, T, NewGreaterValue, msg::greater_than_t<This, T, NewGreaterValue>>;
        ...
        using WithRequired = field<NameTypeT, DWordIndexT, MsbT, LsbT, T, NewRequiredValue, msg::equal_to_t<This, T, NewRequiredValue>>;

        template<T... PotentialValues>
        using WithIn = field<NameTypeT, DWordIndexT, MsbT, LsbT, T, T{}, msg::in_t<This, T, PotentialValues...>>;
        ...
        template<T NewGreaterValue>
        using WithGreaterThanOrEqualTo = field<NameTypeT, DWordIndexT, MsbT, LsbT, T, NewGreaterValue, msg::greater_than_or_equal_to_t<This, T, NewGreaterValue>>;

        template<T NewLesserValue>
        using WithLessThanOrEqualTo = field<NameTypeT, DWordIndexT, MsbT, LsbT, T, NewLesserValue, msg::less_than_or_equal_to_t<This, T, NewLesserValue>>;

        constexpr field(T const & new_value)
            : value{new_value}
        {
            // pass
        }
        ...
        constexpr void insert(DataType & data) const {
            data[DWordIndex] =
                ((static_cast<uint32_t>(value) << LsbT) & field_mask) | (data[DWordIndex] & ~field_mask);
            ...
            if constexpr (MsbT >= 32) {
                ...
                data[DWordIndex + 1] =
                    ((static_cast<uint64_t>(value) >> (32 - LsbT)) & field_mask_dword_1) | (data[DWordIndex + 1] & ~field_mask_dword_1);
            }
            ...
            if constexpr (MsbT >= 64) {
                ...
                data[DWordIndex + 2] =
                    ((static_cast<uint64_t>(value) >> (64 - LsbT)) & field_mask_dword_2) | (data[DWordIndex + 2] & ~field_mask_dword_2);
            }
        }
