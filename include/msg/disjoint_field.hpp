...
namespace msg {
    ...
    public:
        ...
        static constexpr void fits_inside(MsgType msg) {
            fields.for_each([&](auto field){
                field.fits_inside(msg);
            });
        }
        ...
        using WithRequired = disjoint_field<NameTypeT, FieldsT, T, NewRequiredValue, msg::equal_to_t<This, T, NewRequiredValue>>;

        template<T... PotentialValues>
        using WithIn = disjoint_field<NameTypeT, FieldsT, T, T{}, msg::in_t<This, T, PotentialValues...>>;

        template<typename NewRequiredMatcher>
        using WithMatch = disjoint_field<NameTypeT, FieldsT, T, DefaultValue, NewRequiredMatcher>;


        constexpr disjoint_field(T const & new_value)
            : value{new_value}
        {
            // pass
        }
