...
template<typename Field, typename Lookup>
struct index {
    Lookup field_lookup;

    consteval index(Field, Lookup field_lookup_arg) : field_lookup{field_lookup_arg} {}
...
struct indexed_handler : handler_interface<BaseMsgT, ExtraCallbackArgsT...> {
    ...
    constexpr explicit indexed_handler(
        ...
        : index{new_index}
        , callback_entries{new_callbacks}
    {}

    auto is_match(BaseMsgT const & msg) const -> bool final {
        return !index(msg.data).empty();
    }

    void handle(BaseMsgT const &msg, ExtraCallbackArgsT... args) const final {
        auto const callback_candidates = index(msg.data);

        callback_candidates.for_each([&](auto i){
            callback_entries[i](msg, args...);
        });
