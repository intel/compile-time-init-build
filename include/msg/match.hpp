...
namespace match {
    ...
    template<
        ...
        typename ActionType>
    constexpr static event_handler<NameType, MatcherType, ActionType> handle(
        NameType const & name,
    ...
    template<
        typename ActionType>
    constexpr static default_event_handler<ActionType> otherwise(
        ActionType const & action
    ) {
        return {action};
    }
            ...
            if constexpr (hasDefault) {
                ...
                const auto mismatch_descriptions =
                    cib::transform(handlers_tuple, [&](auto handler){
                        return format("    {} - F:({})\n"_sc, handler.name, handler.matcher.describe_match(event));
                    });

                const auto mismatch_description =
                    mismatch_descriptions.fold_left([](auto lhs, auto rhs){
                        return lhs + rhs;
                    });
    ...
    template<bool value>
    constexpr always_t<value> always{};
    ...
    struct any_t {
        ...
        constexpr any_t(MatchersType new_matchers)
        ...
        [[nodiscard]] constexpr auto describe() const {
            const auto matcher_descriptions =
                cib::transform(matchers, [&](auto m){
                    return "("_sc + m.describe() + ")"_sc;
                });

            return matcher_descriptions.fold_left([](auto lhs, auto rhs){
                return lhs + " || "_sc + rhs;
            });
        }
        ...
        [[nodiscard]] constexpr auto describe_match(EventType const & event) const {
            const auto matcher_descriptions =
                cib::transform(matchers, [&](auto m){
                    return format("{:c}:({})"_sc, m(event) ? 'T' : 'F', m.describe_match(event));
                });
    ...
    [[nodiscard]] constexpr auto any(MatcherTypes... matchers) {
        ...
        auto const is_always_true =
            remaining_matcher_tuple.fold_right(sc::bool_<false>, [](auto matcher, auto already_found_always_true){
                using MatcherType = decltype(matcher);
                auto constexpr matcher_is_always_true = std::is_same_v<MatcherType, always_t<true>>;
                return sc::bool_<already_found_always_true || sc::bool_<matcher_is_always_true>>;
            });
        ...
        if constexpr (is_always_true.value) {
            ...
            return remaining_matcher_tuple.apply([&](auto... remaining_matchers){
                return any_t<decltype(remaining_matchers)...>{remaining_matcher_tuple};
            });
        }
    }
    ...
    [[nodiscard]] constexpr auto all(MatcherTypes... matchers) {
        ...
        auto const is_always_false =
            remaining_matcher_tuple.fold_right(sc::bool_<false>, [](auto matcher, auto already_found_always_false){
                using MatcherType = decltype(matcher);
                bool constexpr matcher_is_always_false = std::is_same_v<MatcherType, always_t<false>>;
                return sc::bool_<already_found_always_false || sc::bool_<matcher_is_always_false>>;
            });
        ...
        if constexpr (is_always_false.value) {
            ...
            return remaining_matcher_tuple.apply([&](auto... remaining_matchers){
                return all_t<decltype(remaining_matchers)...>{remaining_matcher_tuple};
            });
        }
    }
