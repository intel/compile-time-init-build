...
[[nodiscard]] constexpr auto set_union(LhsTuple lhs, RhsTuple rhs,
                                       MetaFunc meta_func = self_type{}) {
    return set_operation_impl(
        detail::set_operation_algorithm<true, true, true>{}, meta_func, lhs,
        rhs);
}

template <typename LhsTuple, typename RhsTuple, typename MetaFunc = self_type>
[[nodiscard]] constexpr auto
set_intersection(LhsTuple lhs, RhsTuple rhs, MetaFunc meta_func = self_type{}) {
    return set_operation_impl(
        detail::set_operation_algorithm<false, true, false>{}, meta_func, lhs,
        rhs);
}

template <typename LhsTuple, typename RhsTuple, typename MetaFunc = self_type>
[[nodiscard]] constexpr auto set_difference(LhsTuple lhs, RhsTuple rhs,
                                            MetaFunc meta_func = self_type{}) {
    return set_operation_impl(
        detail::set_operation_algorithm<true, false, false>{}, meta_func, lhs,
        rhs);
}

template <typename LhsTuple, typename RhsTuple, typename MetaFunc = self_type>
[[nodiscard]] constexpr auto
set_symmetric_difference(LhsTuple lhs, RhsTuple rhs,
                         MetaFunc meta_func = self_type{}) {
    return set_operation_impl(
        detail::set_operation_algorithm<true, false, true>{}, meta_func, lhs,
        rhs);
}

template <typename IndexType, IndexType... Indexes, typename Operation>
[[nodiscard]] constexpr auto
make_tuple_from_op(std::integer_sequence<IndexType, Indexes...>, Operation op) {
    return cib::make_tuple(op(std::integral_constant<IndexType, Indexes>{})...);
}

template <typename IndexType, IndexType Size, typename Operation>
[[nodiscard]] constexpr auto make_tuple(std::integral_constant<IndexType, Size>,
                                        Operation op) {
    return make_tuple_from_op(std::make_integer_sequence<IndexType, Size>{},
                              op);
}
...
struct create_demux_tags_t<MetaFunc, cib::tuple_impl<TupleElems...>> {
    constexpr static auto invoke() {
        constexpr auto value = []() {
            constexpr auto const_type_names = cib::detail::create_type_names<
...
template <typename MetaFunc, typename... TupleElems>
constexpr static auto create_demux_tags(MetaFunc,
                                        cib::tuple_impl<TupleElems...>) {
...
[[nodiscard]] constexpr auto demux(MetaFunc, Tuple t) {
    using detail::size_;
...
#define tags create_demux_tags_t<MetaFunc, Tuple>::invoke()
    ...
    return cib::make_tuple(size_<num_bins>, [&](auto bin_index) {
        return cib::make_tuple(
            size_<bin_size[bin_index]>, [&](auto offset_into_bin) {
