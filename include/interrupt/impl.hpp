...
template <> struct read_field<no_field_t> {
    template <typename Hal> CONSTEVAL static auto with_hal() -> bool {
        return true;
    }
};
...
template <> struct clear_field<no_field_t> {
    template <typename Hal> CONSTEVAL static auto with_hal() -> void {}
};
} // namespace detail
...
struct irq_impl : Config {
    ...
    template <typename Hal> static auto run() -> void {
        if constexpr (active) {
...
struct sub_irq_impl : Config {
    ...
    template <typename Hal> static auto run() -> void {
        if constexpr (active) {
            ...
            if (detail::read_field<en_t>::template with_hal<Hal>() and
...
struct shared_irq_impl : Config {
    ...
    template <typename Hal> static auto run() -> void {
        if constexpr (active) {
            using status_policy_t = typename Config::status_policy_t;
            Hal::template run<status_policy_t>(
                Config::irq_number, [] { (Subs::template run<Hal>(), ...); });
        }
    }
};
