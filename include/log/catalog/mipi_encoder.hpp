...
#define LOG(LEVEL, MSG, ...) log::logger_impl::log_impl<LEVEL>(log::format_helper{MSG ## _sc}.f(__VA_ARGS__))
...
namespace log {
    ...
    template<
        typename CriticalSectionRaii,
        typename... LogDestinations>
    struct mipi_encoder {
    private:
        constexpr static auto MAX_ARG_LENGTH = sc::int_<3>;

        template<typename... MsgDataTypes>
        CIB_LOG_NEVER_INLINE static void dispatch_pass_by_args(
        ...
        template<
            log_level level,
            typename IdType,
            typename... MsgDataTypes>
        CIB_LOG_NEVER_INLINE static void log_pass_by_args(
            IdType id,
            MsgDataTypes... msg_data
        ) {
            if constexpr (sizeof...(msg_data) == 0) {
                ...
                uint32_t constexpr normal_header_dw =
                    ...
                    (level <<  4) |
                    0x3;            // mipi sys-t type: catalog

                dispatch_pass_by_args(normal_header_dw, id, msg_data...);
            }
        }

        CIB_LOG_NEVER_INLINE static void dispatch_pass_by_buffer(
            std::uint32_t * msg,
        ...
        CIB_LOG_NEVER_INLINE static void log_pass_by_buffer(
            std::uint32_t * msg,
            std::uint32_t msg_size
        ) {
            msg[0] =
        ...
        CIB_LOG_ALWAYS_INLINE static void dispatch_message(
            MsgDataTupleType msg_data_tuple
        ) {
            constexpr auto msg_size = std::tuple_size_v<MsgDataTupleType>;

            if constexpr (msg_size <= MAX_ARG_LENGTH) {
                std::apply([&](auto... payload) {
                    log_pass_by_args<level>(payload...);
                }, msg_data_tuple);
            } else {
                ...
                std::apply([&](auto... payload) {
                    std::uint32_t full_args_array[full_msg_size] = {0, payload...};
        ...
        CIB_LOG_ALWAYS_INLINE static auto to_tuple(
            IdType id,
            ArgTupleType arg_tuple
        ) {
            auto const normal_headers = std::make_tuple(id);

            auto const normal_payloads =
                std::apply([&](auto... args_pack){
                    return std::make_tuple(static_cast<std::uint32_t>(args_pack)...);
                }, arg_tuple);
    ...
    using logger_impl = mipi_encoder<CIB_LOG_CRITICAL_SECTION_RAII_TYPE, CIB_LOG_DESTINATIONS>;
}